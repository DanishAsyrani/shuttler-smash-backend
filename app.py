...
async def analyze(
        ...
        if res.pose_landmarks:
            ...
            if 0 <= rw.x <= 1 and 0 <= rw.y <= 1:
                ...
                if frame.pts is not None:
                    ...
                    # Fallback to hint fps or approx 30
    ...
    if len(wrist_pts) < 6 or len(timestamps) < 6:
        return SmashResult(
            ...
            notes=f"Insufficient wrist/frames. px/m used={round(PX,1)}"
        )

    # Ensure arrays align (just in case)
    n = min(len(wrist_pts), len(timestamps))
    pts = wrist_pts[:n]
    ts  = timestamps[:n]

    # Compute speed using true per-step dt
    ...
    if len(v) == 0:
        return SmashResult(
            ...
            confidence=0.25,
            frames_used=used_frames,
            notes=f"No motion. px/m used={round(PX,1)}"
        )
    ...
    # Useful diagnostics in notes to sanity-check in app:
